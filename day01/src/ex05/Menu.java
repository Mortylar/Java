...
public class Menu {
    ...
    private AnswerCode readAnswer() {
        int answer =
            ((scanner_.hasNextInt()) ? scanner_.nextInt()
                                     : AnswerCode.DEFAULT_CODE.ordinal());
        ...
        if (scanner.hasNextInt()) {
            ...
            if (scanner.hasNextInt()) {
                ...
                if (scanner.hasNextInt()) {
                    ...
                    try {
                        service_.transactionPerform(recepientID, senderID,
                                                    transferAmount);
    ...
    private void removeTransaction() {
        ...
        if (scanner.hasNextInt()) {
            ...
            if (scanner.hasNext()) {
                ...
                try {
                    Transaction tr =
                        ((service_.getUser(userID)).getTransactionList())
                            .get(transactionID);
                    ...
                    if (tr.getTransactionType() == TransactionType.DEBIT) {
                        System.out.printf(
                            ...
                            tr.getSender().getName(), tr.getSender().getID(),
    ...
    private void transactionValidate() {
        ...
        if (array.length == 0) {
            ...
            for (int i = 0; i < array.length; ++i) {
                ...
                System.out.printf(
                    "%s(id = %d) has an unaknowledged transfer id = %s from %s(id = %d)\n",
